...
export default function SimulationAuditPanel({ audit = [], simulatedBy = "unknown", generatedAt = "" }) {
  return (
    <div className="glass rounded-2xl p-4">
      <p className="text-xs uppercase tracking-[0.25em] text-primary font-bold mb-3">Simulation Audit Layer</p>
      <p className="text-sm text-white/45 mb-3">Latest read-only simulation run by {simulatedBy} at {String(generatedAt).slice(0, 19).replace("T", " ")}.</p>
      <div className="max-h-72 overflow-auto rounded-xl border border-white/[0.06]">
        <table className="w-full text-sm">
          <thead className="bg-white/[0.03]"><tr>{["Ran by", "When", "Action", "Entities", "Projected deltas"].map((h) => <th key={h} className="px-3 py-2 text-left text-[10px] uppercase tracking-wider text-white/40">{h}</th>)}</tr></thead>
          <tbody>
            {audit.map((row, index) => (
            ...
            {audit.length === 0 && <tr><td colSpan={5} className="px-3 py-8 text-center text-white/35">No simulations available.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
