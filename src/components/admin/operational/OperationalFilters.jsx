...
const inputClass = "h-10 rounded-xl bg-white/[0.06] border border-white/[0.1] px-3 text-sm text-white placeholder:text-white/25 outline-none focus:border-primary/50";

export default function OperationalFilters({ filters, onChange, hosts = [], vehicles = [], categories = [], statuses = [], showVehicle = true, showCostRange = false, showReimbursable = false, showTaxDeductible = false, resultCount, totalCount, action }) {
  const set = (key, value) => onChange({ ...filters, [key]: value });

  return (
    <div className="rounded-3xl border border-white/[0.08] bg-white/[0.04] p-4 shadow-card space-y-3">
      ...
      {(resultCount !== undefined || action) && (
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between text-xs text-white/35">
          <span>{resultCount?.toLocaleString?.() ?? resultCount} result{resultCount === 1 ? "" : "s"}{totalCount !== undefined ? ` of ${totalCount.toLocaleString()}` : ""}</span>
          {action}
        </div>
      )}
    </div>
