...
export default function DeviceProvisioningPanel({ devices = [], providers = [] }) {
  ...
  const [manual, setManual] = useState({ provider_key: "moovetrax", unique_id: "", installer_email: "", installer_phone: "" });
  ...
  const importCsv = async (file) => {
    ...
    for (const row of rows) {
      ...
      const response = await base44.functions.invoke("provisionTelematicsDevices", { devices: [{
        ...
        vehicle_id: row.vehicle_id, assigned_installer_email: row.assigned_installer_email, installer_email: row.installer_email || row.assigned_installer_email, installer_phone: row.installer_phone, installation_scheduled_at: row.installation_scheduled_at
      }] });
  ...
  return <Card className="glass"><CardHeader><CardTitle className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-primary" />Device Provisioning</CardTitle></CardHeader><CardContent className="space-y-5"><div className="grid md:grid-cols-6 gap-2"><Input placeholder="Provider key" value={manual.provider_key} onChange={e => setManual(p => ({ ...p, provider_key: e.target.value }))} /><Input placeholder="Unique ID" value={manual.unique_id} onChange={e => setManual(p => ({ ...p, unique_id: e.target.value }))} /><Input placeholder="SIM ICCID" onChange={e => setManual(p => ({ ...p, sim_iccid: e.target.value }))} /><Input placeholder="Installer email" value={manual.installer_email} onChange={e => setManual(p => ({ ...p, installer_email: e.target.value, assigned_installer_email: e.target.value }))} /><Input placeholder="Installer phone" value={manual.installer_phone} onChange={e => setManual(p => ({ ...p, installer_phone: e.target.value }))} /><Button onClick={() => createDevice.mutate(manual)} disabled={!manual.unique_id}>Add Device</Button></div><div className="rounded-2xl border border-border p-4"><input ref={fileRef} type="file" accept=".csv" className="hidden" onChange={e => e.target.files?.[0] && importCsv(e.target.files[0])} /><Button variant="outline" onClick={() => fileRef.current?.click()}><Upload className="h-4 w-4 mr-2" />Bulk Upload CSV</Button><p className="text-xs text-muted-foreground mt-2">Columns: provider_key, unique_id, imei, sim_iccid, provider_device_id, traccar_device_id, model, batch_number, host_id, vehicle_id, assigned_installer_email, installer_email, installer_phone, installation_scheduled_at.</p>{result && <p className="text-xs text-muted-foreground mt-2">Imported {result.created}; skipped {result.skipped} duplicates.</p>}</div><div className="grid md:grid-cols-5 gap-3">{STATUS_GROUPS.map(group => <div key={group.key} className="rounded-2xl border border-border p-3"><p className="text-xs font-bold text-muted-foreground mb-2">{group.label}</p><Badge variant="outline">{devices.filter(group.match).length}</Badge></div>)}</div></CardContent></Card>;
}