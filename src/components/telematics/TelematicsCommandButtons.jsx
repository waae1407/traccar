...
const COMMANDS = [
  ...
  { key: "horn_lights", label: "Horn/Lt", icon: Volume2, capability: "supports_horn", deviceFlag: "horn_light_enabled", roles: ["admin", "host"], tone: "bg-amber-500/15 text-amber-300 border-amber-500/30" },
  { key: "disable_starter", label: "Kill", icon: Zap, capability: "supports_starter_disable", roles: ["admin", "host"], starter: true, tone: "bg-red-500/15 text-red-300 border-red-500/30" },
...
export default function TelematicsCommandButtons({ vehicleId, bookingId, device, provider, role = "admin", booking, allowStarter = false, onResult }) {
  ...
  const visibleCommands = useMemo(() => COMMANDS.filter(cmd => {
    ...
    if (["admin", "host"].includes(role)) return true;
    ...
    <div className="space-y-1.5">
      ...
      {visibleCommands.length > 0 && <div className="grid grid-cols-3 gap-1">
        {visibleCommands.map(cmd => {
          ...
          return <Button key={cmd.key} variant="outline" disabled={!!loading} onClick={() => send(cmd.key)} className={`h-7 justify-center gap-1 rounded-lg border px-1 text-[8px] font-bold ${cmd.tone}`}>{busy ? <Loader2 className="h-3 w-3 animate-spin" /> : <Icon className="h-3 w-3" />}{cmd.label}</Button>;
        })}
      </div>}
    </div>
  );
