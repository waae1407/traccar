  ...
  return (
    <Card className="glass border-white/10">
      <CardContent className="p-5">
        ...
        <Table>
          <TableHeader><TableRow className="border-white/10"><TableHead>Command</TableHead><TableHead>Status</TableHead><TableHead>Sent</TableHead><TableHead>ASCII / HEX</TableHead><TableHead>Failure</TableHead></TableRow></TableHeader>
          <TableBody>
            {(commands || []).map((command) => (
              <TableRow key={command.id} className="border-white/10">
                ...
                <TableCell><Badge className={command.queue_status === 'failed' ? 'bg-red-500 text-white' : 'bg-emerald-500/15 text-emerald-300'}>{command.queue_status || command.status}</Badge></TableCell>
