...
import { Car, Calendar, RefreshCw, AlertCircle, ChevronRight, Check } from "lucide-react";
...
export default function StepVehicle({ vehicles = [], vehicleId, bookingType: initialType, onSelect }) {
  ...
  return (
    <div>
      ...
      {!autoRenew && type === "Weekly" && (
        <div className="flex gap-2 p-3 rounded-xl bg-amber-50 border border-amber-100 mb-4">
          ...
          <p className="text-xs text-amber-700">
            <strong>Auto-renew is off.</strong> Your rental will expire on the end date. Log in and manually renew before it expires to extend.
          </p>
        </div>
      )}
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 p-3 rounded-2xl border-2 text-left transition-all active:scale-[0.98] ${
                  isSelected ? "border-pink-500 bg-pink-50" : "border-gray-100 bg-white shadow-sm hover:border-pink-200"
                }`}
              >
                {v.image_url ? (
                ...
                <div className={`h-6 w-6 rounded-full flex-shrink-0 flex items-center justify-center transition-all ${isSelected ? "bg-pink-500" : "border-2 border-gray-200"}`}>
      ...
      {!startDate && <p className="text-center text-xs text-gray-400 mt-2">Pick a start date to continue</p>}
    </div>
  );
}