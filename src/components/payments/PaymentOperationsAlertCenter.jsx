...
function Kpi({ label, value, icon: Icon, tone }) {
  ...
  return <div className={`rounded-2xl border p-4 ${tones[tone] || tones.blue}`}><Icon className="h-4 w-4 mb-3" /><p className="text-2xl font-black text-white">{value}</p><p className="text-xs opacity-70">{label}</p></div>;
}
  ...
  return (
    <div className={scope === "admin" ? "space-y-6" : "space-y-5"}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
        <Kpi label="Open Critical" value={kpis.critical} icon={ShieldAlert} tone="red" />
        <Kpi label="Failed Payments" value={kpis.failed} icon={CreditCard} tone="yellow" />
        <Kpi label="Chargebacks" value={kpis.chargebacks} icon={AlertTriangle} tone="red" />
        <Kpi label="Payout Issues" value={kpis.payout} icon={DollarSign} tone="blue" />
        <Kpi label="Subscription Issues" value={kpis.subscription} icon={Undo2} tone="yellow" />
        <Kpi label="Resolved This Week" value={kpis.resolved} icon={CheckCircle2} tone="green" />
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden">
        <div className="px-4 py-3 border-b border-white/10 flex items-center justify-between"><p className="font-bold text-white">Alerts</p><p className="text-xs text-white/40">{filtered.length} records</p></div>
        {isLoading ? <div className="p-6 text-white/40">Loading alerts…</div> : filtered.length === 0 ? <div className="p-10 text-center text-white/35">No alerts found.</div> : <div className="divide-y divide-white/10">{filtered.map(a => <div key={a.id} className="p-4 hover:bg-white/[0.03]"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap gap-2 mb-1"><span className="text-[10px] font-black uppercase text-red-300">{a.severity}</span><span className="text-[10px] text-white/35">{a.status}</span><span className="text-[10px] text-white/35">{a.billing_context}</span></div><p className="font-bold text-white">{a.title}</p><p className="text-sm text-white/50 mt-1">{a.message}</p><p className="text-xs text-white/35 mt-2">{a.recommended_action}</p></div><div className="text-right text-xs text-white/40"><p>{a.financial_impact_amount ? `$${Number(a.financial_impact_amount).toFixed(2)}` : "—"}</p><p>{a.created_date ? new Date(a.created_date).toLocaleDateString() : ""}</p></div></div><div className="mt-3 text-[10px] text-white/30 flex flex-wrap gap-3"><span>{a.alert_type}</span>{a.booking_id && <span>booking {a.booking_id.slice(0, 8)}</span>}{a.host_id && <span>host {a.host_id.slice(0, 8)}</span>}{a.stripe_dispute_id && <span>dispute {a.stripe_dispute_id}</span>}</div></div>)}</div>}
      </div>
    </div>
  );
}