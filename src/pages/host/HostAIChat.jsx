...
export default function HostAIChat() {
  ...
  const newConversation = async () => {
    const conv = await base44.agents.createConversation({
      agent_name: AGENT,
      metadata: { name: `Host Chat ${new Date().toLocaleDateString()}` },
    });
  ...
  const SUGGESTIONS = [
    ...
    "Which of my vehicles are currently rented?",
    "What compliance docs are expiring soon?",
    "When is my next payout?",
    "What's my fleet utilization rate?",
    "How does Stripe Connect payout work?",
  ];
...
function MessageBubble({ message, accentClass, accentBg }) {
  const isUser = message.role === "user";
  return (
    <div className={`flex gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
      {!isUser && (
        <div className={`h-7 w-7 rounded-lg ${accentBg} flex items-center justify-center mt-0.5 flex-shrink-0`}>
