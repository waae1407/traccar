...
export default function HostAlert360() {
  ...
  const { data: events = [], isLoading } = useQuery({
    queryKey: ['host-alert360', host?.id],
    queryFn: () => base44.entities.TelematicsSafetyEvent.filter({ host_id: host?.id, visible_to_host: true }, '-first_seen_at', 100),
    enabled: !!host?.id,
  ...
  const columns = [
    ...
    { header: 'Alert Type', accessor: (e) => (
      <div className="flex items-center gap-2">
        {e.host_severity === 'critical' ? <ShieldAlert className="h-4 w-4 text-red-500" /> : <AlertTriangle className="h-4 w-4 text-amber-500" />}
        <div>
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        title="Fleet Alerts" 
        subtitle="Active safety and security events across your fleet"
        icon={AlertCircle}
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
          ...
          <p className="text-2xl font-black text-red-600 mt-2">{events.filter(e => e.is_active && e.host_severity === 'critical').length}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
          <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Active Warnings</p>
          <p className="text-2xl font-black text-amber-600 mt-2">{events.filter(e => e.is_active && e.host_severity === 'warning').length}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
