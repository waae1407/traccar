...
export default function HostAlert360() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['host-alert360-dashboard', host?.id],
    queryFn: () => base44.functions.invoke('getAlert360Dashboard', { host_id: host?.id }).then(r => r.data),
    enabled: !!host?.id,
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Fleet Alerts (Alert360)</h1>
        <p className="text-muted-foreground text-sm mt-1">Active safety and security events across your fleet</p>
      </div>
            ...
            <div key={e.id} className="flex flex-col md:flex-row md:items-center justify-between gap-4 rounded-lg bg-secondary/30 px-4 py-3 text-sm">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-xs">{e.category} · {e.vehicle_display_name}</p>
                </div>
              </div>
