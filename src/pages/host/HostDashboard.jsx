...
export default function HostDashboard() {
  ...
  const { data: readinessResponse } = useQuery({
    queryKey: ["host-first-vehicle-readiness", host?.id],
    queryFn: () => base44.functions.invoke("getHostVehicleSetupReadiness", { host_id: host.id }),
    enabled: !!host?.id,
  });
    ...
    <div className="space-y-5">
      ...
      {hasLiveVehicles ? (
        ...
        <div className="rounded-3xl border border-blue-100 bg-blue-50 p-5">
          <p className="text-sm font-black text-blue-950">Focus on your first vehicle first.</p>
          <p className="mt-1 text-sm text-blue-700">Fleet stats, payouts, maps, reports, maintenance dashboards, and customer analytics will become prominent after your first vehicle is live.</p>
          <div className="mt-4 flex flex-wrap gap-2">
            <Link to="/host/vehicles/setup" className="rounded-xl bg-blue-700 px-4 py-2 text-sm font-black text-white">Add First Vehicle</Link>
            <Link to="/host/installers" className="rounded-xl border border-blue-200 bg-white px-4 py-2 text-sm font-black text-blue-800">Find Installer</Link>
          </div>
        </div>
      )}
    </div>
