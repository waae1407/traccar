  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-4">
      {devices.map(device => {
        ...
        return <Card key={device.id} className="border-gray-100 shadow-sm overflow-hidden"><CardContent className="p-5 space-y-4"><div className="flex items-start justify-between gap-3"><div className="flex gap-3"><div className="h-12 w-12 rounded-2xl bg-pink-50 flex items-center justify-center"><Car className="h-6 w-6 text-pink-600" /></div><div><p className="font-black text-gray-900">{vehicle ? `${vehicle.year} ${vehicle.make} ${vehicle.model}` : device.unique_id}</p><p className="text-xs text-gray-500">{device.provider_key} · {device.unique_id}</p></div></div><Badge className="bg-emerald-50 text-emerald-700 border-emerald-200"><Wifi className="h-3 w-3 mr-1" />{device.online_status || "unknown"}</Badge></div><div className="grid grid-cols-3 gap-2 text-xs"><Info label="Last seen" value={device.last_seen_at ? new Date(device.last_seen_at).toLocaleString() : "—"} /><Info label="Ignition" value={device.ignition_status || "unknown"} /><Info label="Starter" value={device.starter_disabled ? "Disabled" : "Ready"} /></div>{vehicle && <TelematicsCommandButtons vehicleId={vehicle.id} device={device} provider={providers.find(p => p.provider_key === device.provider_key)} role="host" allowStarter={false} />}{recent.length > 0 && <div className="pt-2 border-t border-gray-100"><p className="text-xs font-bold text-gray-400 mb-2">Recent commands</p>{recent.map(c => <p key={c.id} className="text-xs text-gray-500 flex justify-between"><span>{c.command_type}</span><span>{c.status}</span></p>)}</div>}</CardContent></Card>;
      })}
