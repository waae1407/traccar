    ...
    <div className={`rounded-lg border px-3 py-2 text-sm ${statusColor[m.computed_status] || 'bg-secondary/30 border-border'}`}>
      <div className="flex justify-between items-start">
        <div>
          ...
          {m.next_service_mileage && <p className="text-xs text-muted-foreground mt-0.5">Next mileage: {m.next_service_mileage?.toLocaleString()} {m.miles_until_service !== null && `(${m.miles_until_service > 0 ? m.miles_until_service + ' mi left' : 'OVERDUE'})`}</p>}
          {m.linked_expense && <p className="text-xs text-green-400 mt-0.5">✓ Linked expense found</p>}
        </div>
        <div className="text-right">
          ...
          {m.receipt_url && <div className="mt-1"><a href={m.receipt_url} target="_blank" rel="noopener noreferrer" className="text-primary text-xs">Receipt</a></div>}
        </div>
      </div>
    </div>
  );
...
export default function MaintenanceCenter() {
  ...
  const { data: vehicles } = useQuery({ queryKey: ['vehicles_maint'], queryFn: () => base44.entities.Vehicle.list('-created_date', 300) });

  const { data, isLoading } = useQuery({
    queryKey: ['maintenance_center', filterVehicle],
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Smart Maintenance Center</h1>
        <p className="text-muted-foreground text-sm mt-1">HostMaintenanceLog canonical · Next due dates · Mileage alerts · Cost tracking</p>
      </div>
      ...
      <Tabs defaultValue="due_soon">
        <TabsList className="flex-wrap h-auto gap-1">
          {[['due_soon','Due Soon'],['overdue','Overdue'],['scheduled','Scheduled'],['completed','Completed'],['by_vehicle','By Vehicle'],['costs','Costs']].map(([v,l]) => (
        ...
        <TabsContent value="costs" className="mt-4 space-y-2">
          {data?.records?.sort((a, b) => (b.cost || 0) - (a.cost || 0)).slice(0, 30).map(m => (
            <div key={m.id} className="flex justify-between rounded-lg bg-secondary/30 px-3 py-2 text-sm">
              <div><p className="font-medium">{m.service_type?.replace(/_/g,' ')} — {m.vehicle_name}</p><p className="text-muted-foreground text-xs">{m.date}</p></div>
              <p className="text-muted-foreground font-medium">${(m.cost || 0).toFixed(2)}</p>
            </div>
          ))}
        </TabsContent>
      </Tabs>
