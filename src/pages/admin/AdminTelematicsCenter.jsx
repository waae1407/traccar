...
export default function AdminTelematicsCenter() {
  ...
  return <div className="p-4 sm:p-6 space-y-5">
    <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
      ...
      <Badge className="w-fit bg-primary/15 text-primary border-primary/30"><Satellite className="h-3 w-3 mr-1" /> Provider Agnostic</Badge>
    </div>
    ...
    <Card className="glass"><CardHeader><CardTitle className="text-base">Device Registry</CardTitle></CardHeader><CardContent className="space-y-3"><Input placeholder="Search devices, providers, vehicles..." value={query} onChange={e => setQuery(e.target.value)} />{filtered.map(device => <div key={device.id} className="rounded-2xl border border-border bg-card/70 p-4 space-y-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="font-bold">{device.unique_id}</p><p className="text-xs text-muted-foreground">{device.provider_key} · vehicle {device.vehicle_id || "unassigned"}</p></div><div className="flex gap-2"><Badge variant="outline">{device.online_status || "unknown"}</Badge><Badge variant="outline">{device.install_status}</Badge></div></div>{device.vehicle_id && <TelematicsCommandButtons vehicleId={device.vehicle_id} device={device} provider={providers.find(p => p.provider_key === device.provider_key)} role="admin" allowStarter />}</div>)}{filtered.length === 0 && <p className="text-sm text-muted-foreground py-6 text-center">No telematics devices found.</p>}</CardContent></Card>
