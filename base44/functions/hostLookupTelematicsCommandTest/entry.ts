...
const HOST_COMMAND_KEYS = ['locate', 'status', 'lock', 'unlock', 'horn', 'lights', 'horn_lights', 'alarm_pulse'];
const STARTER_COMMAND_KEYS = ['disable_starter', 'restore_starter'];
...
async function getHost(base44, user) {
  const byEmail = await base44.asServiceRole.entities.Host.filter({ email: user.email });
  if (byEmail[0]) return byEmail[0];
  const byUser = await base44.asServiceRole.entities.Host.filter({ user_id: user.id });
...
function isSupported(command, provider, device, host) {
  const allowedKeys = host.telematics_starter_control_enabled === true && device.host_starter_control_enabled === true
    ? [...HOST_COMMAND_KEYS, ...STARTER_COMMAND_KEYS]
    : HOST_COMMAND_KEYS;
  if (!allowedKeys.includes(command.key)) return false;
  if (provider?.[command.capability] !== true) return false;
  if (command.deviceFlag && device?.[command.deviceFlag] !== true) return false;
  if (command.starter && device.production_command_scope !== 'all_supported_commands') return false;
  return true;
}
...
Deno.serve(async (req) => {
  try {
    ...
    if (user.role !== 'host') return Response.json({ error: 'Forbidden: Host access required' }, { status: 403 });

    const host = await getHost(base44, user);
    ...
    if (!device.vehicle_id) return Response.json({ error: 'This device is not assigned to one of your vehicles.' }, { status: 403 });

    const vehicles = await base44.asServiceRole.entities.Vehicle.filter({ id: device.vehicle_id });
    const vehicle = vehicles[0] || null;
    if (!vehicle || vehicle.host_id !== host.id) return Response.json({ error: 'This device is not assigned to one of your vehicles.' }, { status: 403 });
    if (device.host_id && device.host_id !== host.id) return Response.json({ error: 'This device is not assigned to your host account.' }, { status: 403 });

    const provider = await getProvider(base44, device);
    const supported_commands = COMMAND_DEFINITIONS.filter((command) => isSupported(command, provider, device, host));
    ...
    for (const oldSession of sessions) {
      if (oldSession.tested_by === user.email) {
        await base44.asServiceRole.entities.TelematicsDeviceTestSession.update(oldSession.id, {
          status: 'failed',
          completed_at: now,
          notes: [oldSession.notes, 'Closed automatically when a new host command test session started.'].filter(Boolean).join('\n')
        });
      }
    }
    ...
    return Response.json({
      ...
      provider,
      vehicle,
      host: { id: host.id, business_name: host.business_name, full_name: host.full_name },
      session,
      supported_commands,
      execution: {
